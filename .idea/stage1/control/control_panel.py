...
from metadata_processing.metadata_sqlite import parse_gutenberg_metadata, store_metadata_in_db, create_metadata_table, get_metadata_from_db

# Paths for tracking state
CONTROL_PATH = Path("control")
DOWNLOADS = CONTROL_PATH / "downloaded_books.txt"      # Downloaded books tracking
INDEXINGS = CONTROL_PATH / "indexed_books.txt"         # Indexed books tracking
METADATA_STORED = CONTROL_PATH / "metadata_books.txt"  # Metadata books tracking
...
def control_pipeline_step():
    ...
    if ready_to_index:
        ...
        # Run indexing using the imported indexing functions
        try:
